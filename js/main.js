...
function drawField() {
    ...
    ctx.fillRect(FIELD.MARGIN, FIELD.MARGIN, canvas.width - FIELD.MARGIN * 2, canvas.height - FIELD.MARGIN * 2);

    // --- Existing white pitch lines (unchanged code below) ---
    ctx.strokeStyle = "#ffffff";
    ctx.lineWidth = 4;
    ctx.strokeRect(FIELD.MARGIN, FIELD.MARGIN, canvas.width - FIELD.MARGIN * 2, canvas.height - FIELD.MARGIN * 2);
    ctx.beginPath();
    ctx.moveTo(canvas.width / 2, FIELD.MARGIN);
    ctx.lineTo(canvas.width / 2, canvas.height - FIELD.MARGIN);
    ctx.stroke();
    ctx.beginPath();
    ...
    ctx.fillRect(0, goalTop, FIELD.MARGIN, goalH);
    ctx.strokeRect(FIELD.MARGIN, penaltyBoxTop, penaltyBoxW, penaltyBoxH);
    const goalAreaW = 60;
    const goalAreaH = 120;
    const goalAreaTop = (canvas.height - goalAreaH) / 2;
    ctx.strokeRect(FIELD.MARGIN, goalAreaTop, goalAreaW, goalAreaH);
    ctx.fillStyle = "#888";
    ctx.beginPath();
    ctx.arc(FIELD.MARGIN, goalTop, 8, -Math.PI / 2, Math.PI / 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(FIELD.MARGIN, goalBottom, 8, -Math.PI / 2, Math.PI / 2);
    ctx.fill();
    // Right side markings
    ctx.fillStyle = hexToRgba(player2.color, 0.5);
    ctx.fillRect(canvas.width - FIELD.MARGIN, goalTop, FIELD.MARGIN, goalH);
    ctx.strokeRect(canvas.width - FIELD.MARGIN - penaltyBoxW, penaltyBoxTop, penaltyBoxW, penaltyBoxH);
    ctx.strokeRect(canvas.width - FIELD.MARGIN - goalAreaW, goalAreaTop, goalAreaW, goalAreaH);
    ctx.fillStyle = "#888";
    ctx.beginPath();
    ctx.arc(canvas.width - FIELD.MARGIN, goalTop, 8, Math.PI / 2, 3 * Math.PI / 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(canvas.width - FIELD.MARGIN, goalBottom, 8, Math.PI / 2, 3 * Math.PI / 2);
    ctx.fill();
...
const player  = new Car(100, canvas.height / 2, COLORS.PLAYER1, player1Controls);
// Initialize blue car off-screen (hidden in setup mode)
const player2 = new Car(-100, -100, COLORS.PLAYER2, player2Controls);
