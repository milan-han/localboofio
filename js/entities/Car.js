import { CAR, FIELD } from '../config.js';
...
export default class Car {
    constructor(x, y, color, controls) {
        ...
        this.vx = 0;
        this.vy = 0;
        this.acceleration = CAR.ACCELERATION;  // slower throttle
        this.maxSpeed = CAR.MAX_SPEED;        // lower top speed
        this.friction = CAR.FRICTION;
        this.turnSpeed = CAR.TURN_SPEED;
    ...
    update() {
        ...
        const forwardFriction = CAR.FORWARD_FRICTION;                        // always present
        const sideFriction = this.handbrake ? CAR.SIDE_FRICTION_BRAKE : CAR.SIDE_FRICTION_NORMAL;     // VERY grippy unless handbrake pressed
    ...
    checkTrackBounds() {
        if (gameState === "setup") {
            ...
            const fieldMargin = FIELD.MARGIN; // margin equal to pitch white border
        ...
        // Keep in canvas bounds
                if (this.x < FIELD.MARGIN) { this.x = FIELD.MARGIN; this.vx = Math.abs(this.vx) * 0.5; }
                if (this.x > canvas.width - FIELD.MARGIN) { this.x = canvas.width - FIELD.MARGIN; this.vx = -Math.abs(this.vx) * 0.5; }
                if (this.y < FIELD.MARGIN) { this.y = FIELD.MARGIN; this.vy = Math.abs(this.vy) * 0.5; }
                if (this.y > canvas.height - FIELD.MARGIN) { this.y = canvas.height - FIELD.MARGIN; this.vy = -Math.abs(this.vy) * 0.5; }
    }
